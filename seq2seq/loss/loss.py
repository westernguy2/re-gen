...
def score_by_probability(gold, predicted, prob_model = None, vocab = None):
    ...
    gold_len = len(gold_split)
    predicted_split = predicted.split(' ')
    predicted_split = [x for x in predicted_split if x]
    predicted_len = len(predicted_split)
    try:
        max_len = 40
        if len(gold_split) > max_len or len(predicted_split) > max_len:
            return 0
        gold_padded = gold_split+['<pad>']*(max_len-len(gold_split))
        gold_idx_input = [[vocab[i] for i in gold_padded]]

        predicted_padded = predicted_split+['<pad>']*(max_len-len(predicted_split))
        predicted_idx_input = [[vocab[i] for i in predicted_padded]]

        gold_input = prob_model.tokenizer(gold, padding=True, truncation=True, return_tensors="pt", max_length=256)
        predicted_input = prob_model.tokenizer(predicted, padding=True, truncation=True, return_tensors="pt", max_length=256)

        # print(gold)
        # print(predicted)
        # score = prob_model(1, torch.LongTensor(predicted_idx_input).cuda(), torch.LongTensor(gold_idx_input).cuda(), [predicted_len], [gold_len])
        score = prob_model(1, predicted_input.to(torch.device('cuda')), gold_input.to(torch.device('cuda')), [predicted_len], [gold_len])
